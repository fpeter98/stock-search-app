...
const Favourites = () => {
    ...
    useEffect(() => {
        ...
        const favourites: string[] = favouritesStorageItem && JSON.parse(favouritesStorageItem);
        setFavoriteStocks(favourites)
    }, []);

    return (
        <div>
            <h1 className="text-5xl text-yellow-400 text-center py-8">Favourite stocks</h1>
            <div className="flex items-center justify-center flex-col">
                {favoriteStocks.length
                    ? favoriteStocks.map((favouriteStock, index) =>
                        <div className="text-neutral-950 font-bold text-center border-amber-200 border-2 w-1/3 bg-white" key={favouriteStock}>{index + 1}. {favouriteStock}</div>)
